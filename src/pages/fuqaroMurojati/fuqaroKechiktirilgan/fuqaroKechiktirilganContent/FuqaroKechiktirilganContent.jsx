...
const FuqaroKechiktirilganContent = ({ currentUser, permission, ranks }) => {
                                ...
                                <table id="myTable" className="table table-bordered mb-3 table-striped table-hover Tab">
                                    ...
                                    <tbody>
                                        {data?.content?.length > 0 && data?.content?.map((dat, index) => (
                                            <tr key={index}
                                                onClick={() => window.innerWidth <= 768 && history.push(`/fuqaro/murojati/bajarish_ijro/${dat?.id}/kechiktirilgan`)}>
...
export default React.memo(FuqaroKechiktirilganContent)