...
export default function Fishka({ data, params }) {
  const [isMainFishka, setIsMainFishka] = useState(false);
    console.log(data)
  return (
    <div className="col-lg-5">
      <div className="card-body">
        {data?.files?.length > 0 && data?.files?.map((file, ind) => (
          (ind === 0) && (
            <embed
              src={urlFq + "/file/view/" + file?.generatedName}
      ...
      {isMainFishka && (
        <div className="ml-3 mr-3">
          <embed
            src={urlFq + "/document/mainFishka/" + params.id}
            type="application/pdf"
            width="100%"
            height="1000px"
          />
