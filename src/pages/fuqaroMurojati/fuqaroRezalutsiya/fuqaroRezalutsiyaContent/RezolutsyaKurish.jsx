...
export default function RezolutsyaKurish({ currentUser }) {
  ...
  //tasdiqlovchi
  const clickUser = async (e) => {
    setKiruvchiUser(e);
    console.log(kiruvchiUser)
    try {
      const res = await axiosInstance.get(`executor/executors/${e.value}/${JSON.parse(localStorage.getItem('oi'))}`)
      console.log(res)
      let arr = [];
      res.data.forEach((dat) => {
        arr.push({ value: dat?.departmentShortName, label: dat?.departmentShortName, isDisabled: "true" });
    ...
    if (hujjatTuri) {
      if (sanaAsosiy) {
        if (xodimBool) {
          ...
          console.log(nazorat)
          console.log(dateFormatSet(dateCutom))
          console.log(inExecutorResolution)

          console.log({
            id: params.id,
            control: nazorat,
            registrationDateStr: dateFormatSet(dateCutom),
            resolutionAddDto: {
                      ...
                      <div className="card-body">
                        <form>
                          <div className="row">
                            ...
                            <div className={`col-lg-${!data.legalPerson ? '6' : '4'}`}>
                            ...
                            <div className={`col-lg-${!data.legalPerson ? '6' : '4'}`}>
                              <div className="form-group row">
                                <div className="col-lg-12">
                                  <select
                                    data-placeholder="MFY"
                                    disabled
                                    className="form-control select-search form-control-outline select"
                                  >
                                    <option></option>
                                    <option value={data.mahalla?.name} selected={true}>E-pochta</option>
                                    <option value="CO">Fax</option>
                                    <option value="ID">Ichki Tizim</option>
                                    <option value="ID">E-xat</option>
                                  </select>
