...
import FuqaroBajaruvchilarKorish from "../../xomaki/xomakiContent/Ko'rish/korishXomaki/korishXomaki";
import {
...
const FuqaroRezolutionContent = ({currentUser, permission1, ranks}) => {
    ...
    const saveAllData = async (e) => {
        ...
        let formsOutlineElements = {}, allDataForm = [], fileId = [];
        ...
        try {
            console.log({
                ...
                deadline: new Date(startDate1).toISOString().slice(0,10).split('-').reverse().join('.'),
                isResolution: true,
                orgID: JSON.parse(localStorage.getItem('oi')),
            });
            const res = await axiosInstanceFq.post("/appeal/resolution", {
                id: params?.id,
                resolutionContent: resolutionContent?.current?.value,
                problems: problems,
                workPlaceID: JSON.parse(localStorage.getItem("ids")),
            ...
            setTimeout(() => {
                history.push(`/fuqaro/murojati/rezalutsiya`)
            }, [1000 * 1])
        } catch (error) {
            console.log(error);
        }
    ...
    // console.log(new Date(startDate1).toISOString().slice(0,10).split('-').join('.'));
            ...
            <div className="card-body card-body-mobile pt-2">
                <ul className="nav nav-tabs nav-tabs-solid nav-tabs-solid-custom bg-primary NavLink">
                    ...
                    <li className="nav-item">
                        <NavLink exact to={`/fuqaro/murojati/rezalutsiya/${params?.id}`} className="nav-link"
                                                            ...
                                                            <div className="form-group form-group-floating  row">
                                                                <div className="col-lg-12">
                                                                    <div className="position-relative">
                                                                        ...
                                                                        />
                                                                        <label
                                                                            className="label-floating">Tel </label>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
...
export default React.memo(FuqaroRezolutionContent)