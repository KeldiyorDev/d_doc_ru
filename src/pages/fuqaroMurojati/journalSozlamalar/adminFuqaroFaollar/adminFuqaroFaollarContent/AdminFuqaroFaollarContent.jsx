...
const FuqaroFaollarContent = ({ currentUser }) => {
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      try {
        const res = await axiosInstanceFq.get(`journal/active/${JSON.parse(localStorage.getItem('oi'))}?page=0`)
  ...
  const changeInputNumber = async (e) => {
    if (e.code === "Enter" || e.code === "NumpadEnter") {
      try {
        await axiosInstanceFq.patch(`journal/orderNumber`, {
          orders: sortInput
        })
        try {
          const res = await axiosInstanceFq.get(`journal/active/${JSON.parse(localStorage.getItem('oi'))}?page=${selected}`)
          setData(res.data);
        } catch (error) {
          console.log(error);
        }
  ...
  const handlePageClick = async (e) => {
    const res = await axiosInstanceFq.get(`journal/active/${JSON.parse(localStorage.getItem('oi'))}?page=${e.selected}`)
    setData(res.data);
                      ...
                      <tr key={index} style={{ fontSize: "15px" }} className="text-center" id="delete">
                        ...
                        <td>
                          <div
                            ...
                            <Link to={`/fuqaro/murojat/elektron-kitob-ko'rish/${dat.id}`}
                            ...
                            <Link to={`/fuqaro/murojati/elektron-kitob-topshiriqlar/${dat.id}`} className="infoBtn bg-dark" data-popup="tooltip" title="Log">
...
export default React.memo(FuqaroFaollarContent)