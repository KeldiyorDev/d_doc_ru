...
const NazoratdanOlishContent = ({currentUser, permission, ranks}) => {
    ...
    useEffect(() => {
        ...
        const getData = async () => {
            try {
                const res = await axiosInstanceFq.post("/inExecutor/takeControl", {
                    appealTypeID: '%%',
                    receptionTypeID: '%%',
                    deadline: '%%',
                    regNumber: '%%',
    ...
    const SearchData = async () => {
        try {
            const res = await axiosInstanceFq.post(`/inExecuter/takeControl`, {
                        ...
                        <div className="card">
                            <div className="card-body card-body-mobile">
                                ...
                                <>
                                    ...
                                    <table id="myTable"
                                           className="table table-bordered mb-3 table-striped table-hover Tab">
                                        <thead>
                                        ...
                                        {data?.content?.length > 0 && data?.content?.map((dat, index) => (
                                            <tr key={index}
                                                onClick={() => window.innerWidth <= 768 && history.push(`/fuqaro/murojati/bajarish_ijro/${dat?.id}/nazoratdanOlish`)}>
                                      ...
                                      style={{fontSize: "14px"}}>{(d?.firstName && d?.firstName.length > 1) ? ((((d?.firstName[0].toUpperCase() === "S" || d?.firstName[0].toUpperCase() === "C") && d?.firstName[1].toUpperCase() === "H")) ? d?.firstName?.substring(0, 2) + ". " : d?.firstName?.substring(0, 1) + ". ") : ""} {d?.lastName}</span>
                                                ...
                                                <td className="harakat mobile-table-none">
                                                    <div
                                                        ...
                                                        <Link
                                                            to={`/fuqaro/murojati/bajarish_ijro/${dat?.problemID}/nazoratdanOlish`}
