...
import TopRightJournals from "./topRightJournals/TopRightJournals";
...
const FuqaroMurojatiContent = ({currentUser, permission1, ranks}) => {
    ...
    useEffect(() => {
        ...
        const getData = async () => {
            try {
                ...
                setMurojaatSoni(res?.data?.data?.applicantsCount)
    ...
    const saveAllData = async (e) => {
        ...
        console.log(fileId)
        console.log(personRef.current?.props.value)
        if (personRef.current?.props.value[0].value === 1) {
                                                                            ...
                                                                            if (accepted) {
                                                                                if (accepted1) {
                                                                                    if (ConfirmerRef.current.props.value) {
                                                                                        if (fileId.length > 0) {
                                                                                            try {
                                                                                                const res = await axiosInstanceFq.patch("appeal/update", {
                                                                                                    id: params.id,
                                                                                                    ...
                                                                                                    regDate: new Date(startDate).toLocaleDateString(),
                                                                                                    ...
                                                                                                    outDate: new Date(startDateKorres).toLocaleDateString(),
        ...
        } else if (personRef.current?.props.value[0].value === 2) {
            if (orgNameRef.current?.value) {
                ...
                Alert(setAlert, "warning", "Organizatsiya kiritilmagan");
            }
