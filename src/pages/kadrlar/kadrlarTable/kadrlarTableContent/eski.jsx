...
const KadrlarTableContent = ({ currentUser, permission, ranks, activeBranchId, branchs }) => {
  ...
  const AddKadrFunc = async () => {
    ...
    kadrAddData.orgId = Number(oi)
    kadrAddData.branchId = activeBranchId
    kadrAddData.email = emailRef.current.value
    kadrAddData.mobileNumber = numberRef.current.value

    axiosInstanceKadr.post(`create`, kadrAddData).then((res) => {
      Alert(setAlert, "success", "User muvofaqqiyatli qo'shildi");
      setAddModal(false)
      kadrAddData.id = res.data
      dispatch(addUser(kadrAddData))
    }).catch((error) => {
      Alert(setAlert, "warning", error.response.data);
    })
  }
  ...
  const imageUpdate = async () => {
    ...
    axiosInstanceKadr.patch(`update`, {
      id: imageEdit.kadr?.id,
  ...
  return (
    <>
          ...
          <div className="card p-0">
            <div className="card-body p-3" style={{ padding: "10px 20px" }}>
              <div className="row pb-2" style={{ justifyContent: "space-between", gap: "1rem" }}>
                ...
                <div className="d-flex" style={{ gap: "1rem" }}>
                  <button onClick={() => setSearchModal(true)} type="button" title="Qo'shish" className="btn btn-success p-2" style={{ padding: "4px 8px" }}>
      ...
      {addModal && <div id="modal_large" className="modal fade show" tabindex="-1" aria-modal="true" role="dialog" style={{ display: "flex", alignItems: "center", background: "rgba(0, 0, 0, 0.5)" }}>
        <div className="modal-dialog modal-lg" style={{ width: "100%" }}>
          <div className="modal-content">
            <div className="modal-header bg-primary text-white">
            ...
            <div className="modal-body px-2 pb-0" onKeyDown={(e) => keyDown(e, "kadrAdd")}>
              <div className="form-group">
                ...
                <div className="row mb-0 form-group form-group-floating">
                  <div className="col-lg-4">
                    <div className="position-relative">
                      <input
                        ...
                        className="form-control form-control-outline"
                        placeholder="Placeholder"
                        defaultValue={kadrAddData?.birthDate?.split("-")?.reverse()?.join(".")}
                      // disabled={edit ? false : true}
                      />
                      <label
                  ...
                  <div className="col-lg-4">
                    <InputMask mask="+\9\9\8\(99)-999-99-99" style={{ textTransform: "uppercase" }}>
                      {(inputProps) => <div className="form-group form-group-floating mb-0">
                        <div className="position-relative">
                          <input
                            {...inputProps}
